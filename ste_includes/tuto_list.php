...
<?php
	...
	$strSQL = "SELECT * FROM `tuto` WHERE `Subcategory_id`=". $_GET["Subcategory_id"] ." ORDER BY Subcategory_id";
	$rs = mysql_query($strSQL);
	
	$num_vidSQL = mysql_query("SELECT count(Tuto_id) FROM `tuto` WHERE `Subcategory_id`=". $_GET["Subcategory_id"] ."");
	$num_vid = mysql_result($num_vidSQL,0);

	while($row = mysql_fetch_array($rs)) {
	$strTitle = $row['Tuto_desc'];

	$list .= "<li>
                <div class='thumbnail'>
				<a href='reader.php?Tuto_id=" . $row['Tuto_id'] . "&&Subcategory_id=". $row['Subcategory_id']."'>
                ...
                <p align='center'><a href = 'reader.php?Tuto_id=" . $row['Tuto_id'] . "&&Subcategory_id=". $row['Subcategory_id']."'
				class='btn btn-primary btn-small'>Read now</a>
...
<head>
	...
	<link href="ste_content/css/tuto_list.css" rel="stylesheet"> 	
...
<section id="launch" class="modal hide fade">
	...
	<div class="featurette">
        ...
        <p class="lead"><?php echo $meta; ?> <span class="pull-right"><small>Total tutorials : <?php echo $num_vid; ?> </small></span></p>
    </div>
